...
`state.uri.queryParameters` property for every page matched in a stack of routes:

```dart
GoRoute(
  path: '/search',
  builder: (context, state) {
    // use state.uri.queryParameters to get search query from query parameter
    final query = state.uri.queryParameters['query']; // may be null
